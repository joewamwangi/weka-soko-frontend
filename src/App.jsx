  ...
  return <>
    ...
    {!loading&&tab==="notifications"&&<>
      ...
      <div style={{maxWidth:680}}>
        {notifs.map((n,i)=>(
          <div key={i} onClick={()=>{markRead(n.id);if(n.type==="listing_match"&&n.data){try{const d=JSON.parse(n.data);console.log("Opening notification detail:",d);setModal({type:"notification_detail",notification:n,listing:{id:d.listing_id||"unknown",title:d.listing_title||"Listing",description:d.listing_description||"",price:d.listing_price||0,seller_id:d.seller_id,is_unlocked:d.is_unlocked||false,locked_buyer_id:d.locked_buyer_id||null}});}catch(err){console.error("Error parsing notification data:",err);}}else{console.log("Notification type:",n.type,"has data:",!!n.data);}}} style={{display:"flex",gap:14,padding:"16px 0",borderBottom:"1px solid #F5F5F5",cursor:"pointer",opacity:n.is_read?.7:1,transition:"opacity .15s"}}
