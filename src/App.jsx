...
function MpesaModal({ amount, purpose, onSuccess, onClose, allowVoucher, token }) {
  ...
  const [vcode, setVcode] = useState(""); const [discount, setDiscount] = useState(0); const [vLoading, setVLoading] = useState(false);
  ...
  const pay = () => {
    ...
    const iv = setInterval(()=>{ c--; setCd(c); if(c<=0){clearInterval(iv);setStep("done");setTimeout(()=>onSuccess({receipt:"QH"+Math.floor(Math.random()*90000000+10000000),amount:finalAmt}),900);} },1000);
  };
...
export default function App() {
  ...
  return(<>
    ...
    {modal?.type==="detail"&&<DetailModal listing={modal.listing} user={user} token={token} onClose={closeModal} notify={notify}
      ...
      onEscrow={()=>{if(!user){notify("Please sign in first.","warning");return;}const fee=Math.round(modal.listing.price*0.025);setModal({type:"mpesa",amount:modal.listing.price+fee,purpose:`Escrow: ${modal.listing.title}`,allowVoucher:true,onSuccess:({receipt})=>{notify(`🔐 Escrow active! Receipt: ${receipt}`,"success");closeModal();}});}}
    />}
