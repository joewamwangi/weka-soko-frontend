...
function PostAdModal({onClose,onSuccess,token,notify}){
  ...
  const [f,setF]=useState({title:"",category:"",subcat:"",price:"",description:"",reason:"",location:"",county:""});
  const sf=(k,v)=>setF(p=>({...p,[k]:v}));
  ...
  return <Modal title={l.title} onClose={onClose} large footer={
    ...
    <div style={{display:"flex",gap:16,fontSize:12,color:"var(--mut)",marginTop:10,flexWrap:"wrap",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",gap:12}}>
        ...
        {l.expires_at&&<span style={{color:"var(--dim)"}}>⏰ Expires {ago(l.expires_at)} from now</span>}
      </div>
    ...
    <main style={{maxWidth:1180,margin:"0 auto",padding:"32px 22px 80px"}}>
      ...
      <div id="listings-section" style={{background:"var(--surf)",border:"1px solid var(--border)",borderRadius:"var(--r)",padding:"13px 16px",marginBottom:22}}>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:10}}>
          ...
          <button className="btn bp sm" style={{flexShrink:0}} onClick={()=>user?setModal({type:"post"}):setModal({type:"auth",mode:"signup"})}>+ Post Ad</button>
        </div>
