...
const API_URL = process.env.REACT_APP_API_URL || "https://weka-soko-backend-production.up.railway.app";
const FALLBACK_STATS = { users: 1247, activeAds: 384, sold: 2891, revenue: 722750 };

// useLiveStats — fetches real numbers from backend every 60s
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Silently fall back to demo numbers
      }
    };
    fetchStats();
  ...
  return(
    <div>
      <div style={{textAlign:"center",marginBottom:32}}>
        ...
        <p style={{color:"var(--muted)",fontSize:15}}>Real transactions by real Kenyans. Join {liveStats.users.toLocaleString()}+ users already on the platform.</p>
      </div>
      ...
      {page==="home"&&<>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:24,marginBottom:40}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:3,flex:"0 0 auto",width:"min(300px,100%)",borderRadius:"var(--rl)",overflow:"hidden",border:"1px solid var(--border)"}}>
            {[{icon:"👥",label:"Users",val:liveStats.users},{icon:"📦",label:"Active Ads",val:liveStats.activeAds},{icon:"✅",label:"Sold",val:liveStats.sold},{icon:"💰",label:"KSh Transacted",val:liveStats.revenue}].map(s=>(
