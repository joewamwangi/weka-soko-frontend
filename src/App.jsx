...
const ago = ts => { if(!ts)return""; const d=Date.now()-new Date(ts).getTime(); if(d<60000)return"just now"; if(d<3600000)return Math.floor(d/60000)+"m ago"; if(d<86400000)return Math.floor(d/3600000)+"h ago"; return Math.floor(d/86400000)+"d ago"; };
...
function PayModal({type,listingId,amount,purpose,token,user,onSuccess,onClose,notify,allowVoucher}){
  ...
  const [step,setStep]=useState("form"); // form | pushing | polling | done | error
  ...
  const applyVoucher=async()=>{
    try{const v=await api(`/api/vouchers/${vcode.toUpperCase()}`,{},token);setDiscount(v.discount_percent||0);notify(`Voucher applied — ${v.discount_percent}% off!`,"success");}
    catch{notify("Invalid or expired voucher code.","error");}
  };

  const startPayment=async()=>{
    if(!phone||phone.length<10){notify("Enter a valid M-Pesa phone number.","warning");return;}
    setStep("pushing");
    try{
      const endpoint=type==="unlock"?"/api/payments/unlock":"/api/payments/escrow";
      const body={listing_id:listingId,phone:phone.trim()};
      if(discount>0)body.voucher_code=vcode;
      ...
      pollRef.current=setInterval(async()=>{
        c--;setCd(c);
        if(c<=0){clearInterval(pollRef.current);setStep("error");setErrMsg("Payment timed out. Please try again.");return;}
        try{
          ...
          else if(s.status==="failed"){clearInterval(pollRef.current);setStep("error");setErrMsg(s.resultDesc||"Payment failed. Please try again.");}
        }catch{}
      },2000);
  ...
  return <Modal title={type==="unlock"?"🔓 Unlock Buyer Contact":"🔐 Escrow Payment"} onClose={onClose}>
    {step==="form"&&<>
      <div style={{background:"rgba(26,107,56,.06)",border:"1px solid rgba(26,107,56,.2)",borderRadius:"var(--r)",padding:"18px 20px",marginBottom:18}}>
        <div style={{fontSize:11,color:"var(--mut)",marginBottom:2}}>Till Number <strong style={{color:"var(--txt)"}}>5673935</strong> · Weka Soko</div>
        <div style={{fontSize:34,fontWeight:800,color:"var(--a)"}}>{fmtKES(finalAmt)}</div>
        {discount>0&&<div style={{fontSize:12,color:"var(--a)",marginTop:2}}>✓ Voucher: {discount}% off applied</div>}
        <div style={{fontSize:13,color:"var(--mut)",marginTop:4}}>{purpose}</div>
      </div>
      ...
      {finalAmt===0
        ?<button className="btn bp lg" style={{width:"100%"}} onClick={()=>{setStep("done");setTimeout(()=>onSuccess({receipt:"VOUCHER-"+Date.now(),amount:0}),400);}}>Redeem for Free →</button>
        :<button className="btn bp lg" style={{width:"100%"}} onClick={startPayment} disabled={phone.length<10}>Send M-Pesa Request → {fmtKES(finalAmt)}</button>}
      <p style={{fontSize:11,color:"var(--dim)",textAlign:"center",marginTop:12}}>You'll receive an M-Pesa push notification on your phone. Enter your PIN to confirm.</p>
    </>}
    {step==="pushing"&&<div style={{textAlign:"center",padding:"30px 0"}}>
      <div style={{marginBottom:18}}><Spin s="48px"/></div>
      <h3 style={{fontWeight:700,marginBottom:8}}>Sending M-Pesa Request...</h3>
      <p style={{color:"var(--mut)",fontSize:14}}>Wait for the STK push on <strong>{phone}</strong></p>
    </div>}
    {step==="polling"&&<div style={{textAlign:"center",padding:"30px 0"}}>
      <div style={{fontSize:64,marginBottom:14}}>📱</div>
      <h3 style={{fontWeight:700,marginBottom:8}}>Enter Your M-Pesa PIN</h3>
      <p style={{color:"var(--mut)",fontSize:14,marginBottom:18}}>Check your phone and enter PIN to pay <strong>Till 5673935</strong></p>
      <div style={{fontSize:44,fontWeight:800,color:"var(--a)",marginBottom:8}}>{cd}s</div>
      <div className="progress"><div className="progress-bar" style={{width:`${(cd/60)*100}%`}}/></div>
      <p style={{fontSize:12,color:"var(--dim)",marginTop:12}}>Waiting for confirmation...</p>
    </div>}
    {step==="done"&&<div style={{textAlign:"center",padding:"30px 0"}}>
      <div style={{fontSize:64,marginBottom:14}}>✅</div>
      <h3 style={{color:"var(--a)",fontWeight:700,marginBottom:8}}>Payment Confirmed!</h3>
      <p style={{color:"var(--mut)",fontSize:14}}>Receipt sent to your inbox and email.</p>
    </div>}
...
function ChatModal({listing,user,token,onClose,notify}){
  ...
  useEffect(()=>{
    api(`/api/chat/${listing.id}`,{},token).then(msgs=>setMessages(Array.isArray(msgs)?msgs:[])).catch(()=>{}).finally(()=>setLoading(false));
    ...
    socket.on("new_message",msg=>setMessages(p=>[...p,{...msg,direction:msg.sender_id===user.id?"me":"them"}]));
  ...
  const send=()=>{
    ...
    socketRef.current.emit("send_message",{listingId:listing.id,body:text.trim()});
    setText("");
  };

  return <Modal title={`💬 ${listing.title}`} onClose={onClose} large>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
      <div style={{width:8,height:8,borderRadius:"50%",background:connected?"var(--a)":"var(--dim)"}}/>
      <span style={{fontSize:12,color:"var(--mut)"}}>{connected?"Connected":"Connecting..."}</span>
      <span style={{fontSize:12,color:"var(--mut)",marginLeft:"auto"}}>Messages are moderated for safety</span>
    </div>
    <div className="chat-wrap">
      <div className="chat-msgs">
        {loading?<div style={{textAlign:"center",padding:20}}><Spin/></div>
          :messages.length===0?<div style={{textAlign:"center",padding:24,color:"var(--mut)",fontSize:13}}>No messages yet. Start the conversation! 👋</div>
          :messages.map((m,i)=>(
            <div key={i} className={`chat-msg ${m.direction||"them"}${m.is_blocked?" blocked":""}`}>
              <div>{m.is_blocked?<em>🚫 Removed: {m.block_reason}</em>:m.body}</div>
      ...
      <div className="chat-input">
        <input className="inp" style={{flex:1}} placeholder="Type a message..." value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()}/>
        <button className="btn bp sm" onClick={send} disabled={!text.trim()}>Send</button>
      </div>
    </div>
    {!listing.is_unlocked&&<div className="alert ay" style={{marginTop:12,fontSize:12}}>🔒 Contact info is hidden until the seller unlocks. Don't share numbers in chat — they'll be blocked.</div>}
  </Modal>;
}
  ...
  return <Modal title="My Account" onClose={onClose} xl>
    ...
    {!loading&&tab==="inbox"&&<>
      ...
      {threads.length===0?<div className="empty"><div style={{fontSize:48,marginBottom:12,opacity:.3}}>💬</div><p>No conversations yet</p></div>
        :threads.map((t,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:"var(--sh)",borderRadius:"var(--rs)",marginBottom:8,border:"1px solid var(--border)",cursor:"pointer"}} onClick={()=>setSelectedListing({id:t.listing_id,title:t.title,seller_id:null,is_unlocked:false})}>
            <div style={{width:44,height:44,borderRadius:"50%",background:"var(--a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,color:"#fff",flexShrink:0}}>
              {t.other_party_anon?.charAt(0)||"?"}
            </div>
            <div style={{flex:1}}>
              <div style={{fontWeight:600,fontSize:14}}>{t.title}</div>
              <div style={{fontSize:12,color:"var(--mut)",marginTop:2}}>{t.last_message?.slice(0,60)||"No messages"}</div>
            </div>
            <div style={{textAlign:"right",flexShrink:0}}>
              <div style={{fontSize:11,color:"var(--dim)"}}>{ago(t.last_message_at)}</div>
              {parseInt(t.unread_count||0)>0&&<div style={{background:"var(--red)",color:"#fff",borderRadius:10,fontSize:10,fontWeight:700,padding:"1px 6px",marginTop:4}}>{t.unread_count}</div>}
            </div>
          </div>
        ))}
    ...
    {!loading&&tab==="settings"&&<>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <div style={{padding:"16px",background:"var(--sh)",borderRadius:"var(--rs)",border:"1px solid var(--border)"}}>
          ...
          <div style={{fontSize:14}}><strong>Role:</strong> {user.role}</div>
        </div>
        ...
        <button className="btn br2" style={{justifyContent:"flex-start",gap:10}} onClick={()=>{if(window.confirm("Delete your account? This cannot be undone.")){}; notify("Contact support@wekasoko.co.ke to delete your account.","info");}}>🗑 Delete Account</button>
      </div>
    </>}
    ...
    {showPayModal&&<PayModal
      ...
      user={user}
      allowVoucher={true}
      onSuccess={()=>{setShowPayModal(null);setListings(p=>p.map(l=>l.id===showPayModal.id?{...l,is_unlocked:true}:l));notify("🔓 Buyer contact unlocked!","success");}}
...
export default function App(){
  ...
  return <>
    ...
    {modal?.type==="pay"&&user&&<PayModal
      ...
      onSuccess={()=>{closeModal();notify(modal.payType==="unlock"?"🔓 Buyer contact revealed!":"🔐 Escrow activated!","success");}}
